...
const Button = ({ label, className, onClickfun }) => (
    <button onClick={onClickfun} className={className}>{label}</button>
)
    ...
    return (
        <section className='pt-10 bg-[url(/images/hero-banner.png)] bg-cover bg-no-repeat bg-fixed bg-[center_70%] sm:bg-[center_60%] md:bg-[center_80%] lg:bg-[center_10%] min-h-screen lg:min-h-[860px] flex items-center' id='home' >
            <div className="container mx-auto px-4 text-gray-50">
                ...
                <Button onClickfun={navigateToSearch} label='Search Property' className="px-8 py-4 mt-30 rounded-full text-white font-semibold bg-white/10 backdrop-blur-md border border-white/30 hover:bg-white/20 transition-all duration-300 shadow-lg hover:scale-105" />
            </div>
        </section>
    )
}
